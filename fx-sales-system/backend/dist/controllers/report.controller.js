...
class ReportController {
    ...
    static async getMonthlyReport(req, res) {
        try {
            ...
            if (!year || !month) {
                throw new error_middleware_1.AppError('Year and month are required', 400);
            }
            const startDate = new Date(Number(year), Number(month) - 1, 1);
            const endDate = new Date(Number(year), Number(month), 0);
            endDate.setHours(23, 59, 59, 999);
            // 获取交易数据
            const transactions = await db_service_1.default.transaction.findMany({
                where: {
                    transaction_date: {
                        gte: startDate,
                        lte: endDate
            ...
            throw new error_middleware_1.AppError('Failed to generate monthly report', 500);
        }
    }
    // 导出月度报表为Excel
    static async exportMonthlyReport(req, res) {
        try {
            ...
            res.setHeader('Content-Disposition', `attachment; filename=FX_Sales_Report_${year}_${month}.xlsx`);
            // 写入响应
            ...
            throw new error_middleware_1.AppError('Failed to export report', 500);
        }
    }
}
